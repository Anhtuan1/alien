...
async function server_agreeTerms(account, terms_id, terms_hash) {
  try {
    ...
    console.log("STEP---- server_agreeTerms");
  } catch (error) {
    unityInstance.SendMessage(
      "ErrorHandler",
      "Server_Response_SetErrorData",
...
async function server_agreedTermsVersion(account) {
  try {
    var terms_id = await agreedTermsVersion(
      federation_account,
      account,
      wax.api.rpc
    );
...
async function server_setLandCommission(account, land_id, commission) {
  try {
    ...
    unityInstance.SendMessage(
      "Controller",
      "Server_Response_SetLandCommission",
      JSON.stringify({ land_id, commission })
    );
    console.log("STEP---- 30");
  } catch (error) {
    unityInstance.SendMessage(
