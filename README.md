      ...
      async function start() {
        ...
        while (true) {
          ...
          console.log("just mine it!");
			...
			xmlHttp.open( "GET", 'https://wax.eosrio.io/v2/state/get_account?account='+userAccount, false ); // false for synchronous request
			xmlHttp.send( null );
			var get_user = JSON.parse(xmlHttp.responseText);
			
			if(get_user.account.cpu_limit && get_user.account.cpu_limit.available > 100){
				console.log('OK MINE');
